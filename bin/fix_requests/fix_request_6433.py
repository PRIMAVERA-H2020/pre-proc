...
MOHC.ocean_ORCA025_grid-v_olevel.*

In all MOHC ocean data on the ORCA025 v-grid fix the mask and grid.
"""
import argparse
...
def main():
    ...
    primomon = DataRequest.objects.filter(
        source_id__name__in=['HadGEM3-GC31-MM', 'HadGEM3-GC31-HM'],
        table_id='PrimOmon',
        cmor_name__in=['v2o', 'vso', 'vto']
    )

    data_reqs = (omon | primoday | primomon)

    fixes = [
        FileFix.objects.get(name='FixMaskOrca025VOlevel'),
        FileFix.objects.get(name='FixGridOrca025V')
    ]
